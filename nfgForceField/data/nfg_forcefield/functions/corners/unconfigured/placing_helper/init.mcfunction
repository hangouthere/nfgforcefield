...
execute as @e[tag=ff_prot_mob,tag=ff_corner,tag=ff_start,tag=!ff_configured,tag=!ff_init,sort=nearest,limit=1] run scoreboard players operation #_areaMin _ff_calcs = MinAreaVolume _ff_calcs
execute as @e[tag=ff_prot_build,tag=ff_corner,tag=ff_start,tag=!ff_configured,tag=!ff_init,sort=nearest,limit=1] run scoreboard players operation #_areaMin _ff_calcs = MinAreaPerim _ff_calcs
