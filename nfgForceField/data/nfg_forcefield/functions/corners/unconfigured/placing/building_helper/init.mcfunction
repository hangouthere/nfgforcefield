...
execute as @p run loot spawn ~ ~ ~ loot nfg_forcefield:get_player_head
...
# Kill player head
execute as @e[type=item,nbt={Item:{id:"minecraft:player_head"}},sort=nearest,limit=1] run kill @s
...
execute store result storage nfg:forcefield operations.meta.ff_start.IDs.player int 1 run scoreboard players get @p[tag=ff_building] nfg_player_id
# corner.start[x,y,z]
data modify storage nfg:forcefield operations.meta.ff_start.corner.start set from entity @s Pos

# Copy initial data to corner
data modify entity @e[tag=ff_corner,tag=ff_start,sort=nearest,limit=1] ArmorItems[0].tag set from storage nfg:forcefield operations.meta.ff_start

# Use a ff_building_helper to store data, future ticks will keep this stand near the player, and thus keep data nearby
execute at @p run summon armor_stand ~ ~ ~ {Small:1b,Invisible:1b,Invulnerable:1b,Tags:["ff_building_helper"],ArmorItems:[{id:"minecraft:dirt",Count:1b},{},{},{}]}
