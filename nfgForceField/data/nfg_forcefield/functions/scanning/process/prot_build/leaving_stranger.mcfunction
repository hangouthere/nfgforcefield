tag @s remove ff_leaving_stranger
...
execute store result score #str_len ff_calcs run data get storage nfg:forcefield scanner.current.messages.stranger.leaving.serverChat
execute unless score #str_len ff_calcs matches 0 run tellraw @s {"nbt":"scanner.current.messages.stranger.leaving.serverChat","storage":"nfg:forcefield"}
